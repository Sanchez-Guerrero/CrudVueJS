...
export async function fetchTodos({ commit }) {
  try {
    ...
    console.log("La petición para obtener todos ha finalizado");
  }
}

/* dispatch = esta en el contexto del primer argumento de una accion y se * ejecuta para ejecutar otra accion
dispatch('fetchTodos'); //para que vuelva a cargar el fetch, pero con el nuevo registro
*/
...
export async function updateTodoStatus({ commit, dispatch }, todo) {
  try {
    ...
    console.log(
      "La petición para actualizar el estado de un todo ha finalizado"
    );
  }
}
